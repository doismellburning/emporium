...
def fetch_latest_version(package_name):
    ...
    if created:
        django_rq.enqueue("fetch_setuppy", package_name, pv.version)
...
def fetch_setuppy(package_name, package_version):
    ...
    django_rq.enqueue("parse_dependencies", package_name, package_version)
...
def parse_dependencies(package_name, package_version):
    ...
    for dependency in dependencies:
        django_rq.enqueue("fetch_latest_version", dependency.package.name)
